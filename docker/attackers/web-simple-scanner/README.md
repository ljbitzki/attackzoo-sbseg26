# Attack "Web Simple Scanner"

> Simplified scanner for known web vulnerabilities.
...
```bash
python3 attackzoo.py run web_simple_scanner --target <TARGET>
```

Run the container directly for isolated validation:

```bash
docker run --rm -d --name attack-web-simple-scanner attack-web-simple-scanner:latest "<TARGET_IP>" "8080"
```
