# Attack "Port Scanner UDP"

> UDP port scan of the target.
...
```bash
python3 attackzoo.py run recon_port_scanner_udp --target <TARGET>
```

Run the container directly for isolated validation:

```bash
docker run --rm -d --name attack-port-scanner-udp attack-port-scanner-udp:latest "<TARGET_IP>"
```
