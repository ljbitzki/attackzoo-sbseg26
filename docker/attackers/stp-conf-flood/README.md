# Attack "STP Config Flood"

> BPDU (Bridge Protocol Data Unit) packet flood with STP topology reconfiguration information and random MAC addresses.
...
```bash
python3 attackzoo.py run net_stp_conf_flood --target <TARGET>
```

Run the container directly for isolated validation:

```bash
docker run --rm -d --name attack-stp-conf-flood attack-stp-conf-flood:latest
```
