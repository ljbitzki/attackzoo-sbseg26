...
> TCP packet flood with the RST flag set.
...
```bash
python3 attackzoo.py run dos_rst_flood --target <TARGET>
```

Run the container directly for isolated validation:

```bash
docker run --rm -d --name attack-rst-flood attack-rst-flood:latest "<TARGET_IP>" "8080"
```
