# Attack "XRCE-DDS Malformed Injection"

> Injection of malformed XRCE-DDS publications or messages against the agent to trigger errors, exceptions, or crashes.
...
```bash
python3 attackzoo.py run iot_xrce_dds_malformed_inject --target <TARGET>
```

Run the container directly for isolated validation:

```bash
docker run --rm -d --name attack-xrce-dds-malformed-inject attack-xrce-dds-malformed-inject:latest "<TARGET_IP>" "8888"
```
