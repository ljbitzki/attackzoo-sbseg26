...
In campaign `experiments/60att_5runs_l0l1l2l3`, this document consolidates the execution of attack `iot_coap_get_flood`. In the local catalog, the attack is described as: Burst of CoAP GET requests against the target CoAP/IoT service to overload the application layer. The documentation below uses only artifacts already present in the repository, mainly the tables and figures from `experiments/60att_5runs_l0l1l2l3/iot_coap_get_flood`.
...
- Campaign artifacts: `experiments/60att_5runs_l0l1l2l3/iot_coap_get_flood`
