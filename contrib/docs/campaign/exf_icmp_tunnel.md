...
In campaign `experiments/60att_5runs_l0l1l2l3`, this document consolidates the execution of attack `exf_icmp_tunnel`. In the local catalog, the attack is described as: TCP port 22 (SSH) tunnel over ICMP (pings). The documentation below uses only artifacts already present in the repository, mainly the tables and figures from `experiments/60att_5runs_l0l1l2l3/exf_icmp_tunnel`.
...
- Campaign artifacts: `experiments/60att_5runs_l0l1l2l3/exf_icmp_tunnel`
